...
限制和单一 workspace bind mount 启动。POSIX 映射当前非 root 宿主 UID/GID并拒绝 root 身份；Docker
Desktop 使用固定非 root 容器用户。容器使用唯一名称和 create/start/remove 生命周期，超时后强制删除。Docker CLI、daemon
